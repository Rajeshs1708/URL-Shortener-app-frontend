...
function Profile () {
  ...
  const getUrlList = () => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/api/getUrl`)
      .then(response => {
  ...
  const deleteUrl = urlId => {
    axios
      .delete(`${process.env.REACT_APP_BASE_URL}/api/deleteUrl/${urlId}`)
      .then(response => {
        if (response) {
          getUrlList()
        }
  ...
  const handleSubmit = e => {
    ...
    try {
      if (longUrl) {
        axios
          .post(`${process.env.REACT_APP_BASE_URL}/api/create`, {
            longUrl: longUrl
          })
          .then(res => {
            ...
            >
              ...
              <div className='card-body text-secondary'>
                <h6 className='card-title p-2'>
                  Short URL :{' '}
                  <a
                    href={`${process.env.REACT_APP_BASE_URL}/api/${url.shortUrl}`}
                    className='urlHeading'
                    target='_blank'
                    without
                    rel='noreferrer'
                  >
                    {process.env.REACT_APP_BASE_URL}/api/{url.shortUrl}
                  </a>
                </h6>
                <h6 className='p-2'>
                  Long URL :{' '}
